...
class DDQN:
    ...
    def learn(self):
        ...
        next_q = self.network(s_prime).detach()
        next_action = next_q.argmax(dim=1)

        q_target = self.target_network(s_prime).detach() # next State 투입

        td_target = r + (1. - terminated) * self.gamma *  q_target.gather(1, next_action.unsqueeze(1)) #next state 중 a'해당하는 값을 가져옴
